...
//create object to read JSON config file in Groovy
def jsonfile = new File("config.json")
JSONObject jsonContent = new JSONObject("config.json");
String resourceName = "config.json"; //json file
...
println(object)
//def jsonfile = new File(config_fn)
...
/*
println(nChannels_vert)
println(nChannels_horiz)
println(channel_size_horiz)
println(channel_size_vert)
*/
...
for (a in [PP1, PP3, PP4, PP2].transpose().collect { [it[0], it[1], it[2], it[3]] }) {
    ...
    */
    ...
    if ((last_horiz == horiz_val) && (spotpoly.getCentroidY() < last_y)) {
        flip_flag = true
        break
    }
    
    last_horiz = horiz_val
    last_vert = vert_val
    last_y = spotpoly.getCentroidY()
